...
import metrics as M
from data import COLOUR_MODES, IMAGENET_MEAN, IMAGENET_STD, load_split
from models import ModelConfig, build_model, coral_to_probabilities
from ood import STAT_FEATURES, FundusGate, image_statistics, worst_statistics
from retina import standardise
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--decision", type=Path, default=None,
                        help="bundle ensemble V3+V4+V5 từ decision.py; mặc định dùng "
                             f"{DEFAULT_DECISION} khi không truyền --checkpoint")
    ...
    if args.decision is None and not (args.checkpoint or args.ensemble or args.evaluate):
        args.decision = DEFAULT_DECISION
...
def main() -> int:
    ...
    if args.batch:
        if ensemble is None:
            raise SystemExit("--batch cần --decision hoặc --checkpoint")
