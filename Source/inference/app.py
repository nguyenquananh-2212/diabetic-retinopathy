...
from demo import (  # noqa: E402
    Ensemble, Grader, batch_summary, compare_to_truth,
...
# The frozen model: V3+V4+V5 blended at the eye, thresholds from decision.py.
...
class Backend:
    def __init__(self, checkpoints: Optional[Path], gate_path: Path,
                 decision: Optional[Path] = None) -> None:
        if decision is not None:
            self.ensemble = Ensemble.from_decision(decision)
            self.graders = self.ensemble.graders
            self.calibrated = True
            fit_hint = f"--decision {decision}"
        else:
            ...
            fit_hint = f"--checkpoint {' '.join(str(p) for p in paths)}"

        if not gate_path.is_file():
            raise SystemExit(
                f"không có cổng lọc tại {gate_path}. Chạy trước:\n"
                f"  python Source/inference/demo.py --fit-gate {fit_hint} --gate {gate_path}"
            )
...
def boundary_html(band: Dict) -> str:
    ...
    for b in band["boundaries"]:
        ...
        blocks.append(
            ...
            # The shaded band is where the demo does not decide alone.
...
def grade_html(band: Dict) -> str:
    ...
    if band.get("rule"):
        extra = ""  # the cut rule is the pipeline, not a detour from argmax
...
def main() -> int:
    ...
    parser.add_argument("--decision", type=Path, default=None,
                        help=f"bundle mô hình chốt; mặc định {DEFAULT_DECISION} nếu có")
    parser.add_argument("--checkpoints", type=Path, default=None,
                        help="chế độ cũ: thư mục *_best.pt + ensemble_tta.json (bỏ qua bundle)")
    ...
    if decision is None and args.checkpoints is None and DEFAULT_DECISION.is_file():
        decision = DEFAULT_DECISION
    gate = args.gate or (decision.parent / "fundus_gate.json" if decision else DEFAULT_GATE)
    backend = Backend(args.checkpoints or DEFAULT_CHECKPOINTS, gate, decision)
